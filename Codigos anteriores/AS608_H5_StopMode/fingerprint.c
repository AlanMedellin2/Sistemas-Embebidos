...
uint16_t baud_rate = 9600;  ///< The UART baud rate (set by getParameters)
...
uint16_t check_fingerprint() {
    ...
    if (rpacket.data[0] != FINGERPRINT_OK)
        return -3;  // no hay dedo o imagen mala
    ...
    data[1] = 2;           // buscar usando buffer 2
    data[2] = 0x00;        // ID start high byte
    data[3] = 0x00;        // ID start low byte
    data[4] = (capacity >> 8);
    data[5] = (capacity & 0xFF);
    setup_packet(data, 6);
    receive();

    uint8_t code = rpacket.data[0];

    if (code == FINGERPRINT_NOTFOUND) {
        return -1;
    }
    else if (code != FINGERPRINT_OK) {
        return -2;
    }

    // SI SE ENCONTRÓ — EL ID ESTÁ AQUÍ:
    uint16_t fingerID = ((uint16_t)rpacket.data[1] << 8) | rpacket.data[2];

    // Opcional: SCORE = data[3:4]
    return (int16_t)fingerID;
}
