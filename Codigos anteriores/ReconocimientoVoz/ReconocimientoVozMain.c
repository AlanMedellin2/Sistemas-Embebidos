...
volatile float alan[CARACTERISTICAS]={0.1261,1271.9584,917.9783,2294.5500};
volatile float diego[CARACTERISTICAS]={0.1453,1453.2654,895.9827,2417.8501};
volatile float adrian[CARACTERISTICAS]={0.1278,1260.2662,899.2557,2263.0500};
...
//Nueva entrada de prueba
volatile float prueba[CARACTERISTICAS]={0.0652, 3100.4428, 420.3381, 1200.5592};


volatile float similitud=10;
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  while (1)
  {
	  switch(devState){
		    ...
		    distancia_relativa(adrian, prueba, distAdrian);
		    distancia_relativa(diego, prueba, distDiego);
		    distancia_relativa(alan, prueba, distAlan);


		    similitud=definirPersona(distAdrian, distAlan, distDiego);
...
/*
void aplicarLog(float *x) {
    for(int i = 1; i < CARACTERISTICAS; i++) {
        x[i] = log10f(x[i]);
    }
}

float distancia(const float *a, const float *b) {
    float dist = 0.0f;
...
/*
float distanciaNormalizada(const float *ref_mean,
                           const float *input,
                           const float *std_vector)
{
    float dist = 0.0f;
    for (int i = 0; i < CARACTERISTICAS; i++) {
        float z = (input[i] - ref_mean[i]) / std_vector[i];
        dist += z * z;
    }
    return sqrtf(dist);
}*/
...
/*
float corrPearson(const float *datosPersona, const float *nueva){

	float meanX=0,meanY=0,denA=0,denB=0,num=0;

	arm_mean_f32(datosPersona, CARACTERISTICAS, &meanX);
	arm_mean_f32(nueva, CARACTERISTICAS,&meanY);

	float dataX=0,dataY=0;

	for(int i=0;i<CARACTERISTICAS;i++){
		dataX=datosPersona[i]-meanX;
		dataY=nueva[i]-meanY;
