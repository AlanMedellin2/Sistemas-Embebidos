...
/* Use 16-bit buffer (ADC 16-bit) */
volatile uint16_t mic[32000] = {0};   // DMA destination
float32_t mic_V[32000] = {0};         // processed voltages

/* pointer no es estrictamente necesario si usamos indices */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if(GPIO_Pin == USER_BTN_Pin){
		...
		if (devState == MEASURE){ //Si ya llegó al ultimo estado, vuelve al estado 0 IDLE.
			devState = IDLE;
