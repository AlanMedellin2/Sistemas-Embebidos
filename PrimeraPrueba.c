...
void Yellow_Task(void *argument){
	uint16_t actualDelay = 200;
	uint8_t newIndex = 0;
	const uint16_t speeds [5] = {200, 400, 600, 800, 1000};
	for(;;){
		if (osMessageQueueGet(queueSpeedHandler, &newIndex, NULL, 0) == osOK){
			actualDelay = speeds[newIndex];
		}
...
void Red_Task(void *argument){
	...
	for(;;){
		if(osSemaphoreAcquire(buttonSemHandle, osWaitForever) == osOK){
			...
			osDelay(300);
			osMessageQueuePut(queueSpeedHandler, &counter, 0, 0); //No timeout
			counter = (counter + 1) % 5;
		}
	}
}
