...
const osThreadAttr_t greenTask_attr = {
	.name = "greenTask",
	.stack_size = 128 * 4,
	.priority = (osPriority_t) osPriorityLow,
};
osThreadId_t yellowTaskHandle;
const osThreadAttr_t yellowTask_attr = {
	.name = "yellowTask",
	.stack_size = 128 * 4,
...
const osThreadAttr_t redTask_attr = {
	.name = "redTask",
	.stack_size = 1024 * 4,
	.priority = (osPriority_t) osPriorityNormal,
};
//CREACIÓN DEL HANDLER SEMÁFORO BINARIO!
...
void Yellow_Task(void *argument);
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  /* add semaphores, ... */
  //CREACIÓN DE MI SEMÁFORO
  buttonSemHandle = osSemaphoreNew(1, 0, &buttonSem_attr);
  ...
  greenTaskHandle = osThreadNew(Green_Task, NULL, &greenTask_attr);
  yellowTaskHandle = osThreadNew(Yellow_Task, NULL, &yellowTask_attr);
  redTaskHandle = osThreadNew(Red_Task, NULL, &redTask_attr);
  /* add threads, ... */
...
void Red_Task(void *argument){
	...
	for(;;){
		...
		promedio = 0;
		items = osMessageQueueGetCount(queueSpeedHandler);

		if(items>0){
			for(int i=0;i<items;i++){
				if (osMessageQueueGet(queueSpeedHandler, &newValue, NULL, 0) == osOK){
					array[i]=(float32_t)newValue;
				}
			}
			arm_mean_f32(array, items, &promedio);
		}
		...
		if (osMutexAcquire(uartMutexHandle, 500) == osOK){
			if (items == 0)
			{
				sprintf((char*)msg2, "No hay valores disponibles.\r\n");
			}
			else
			{
				//Sacar promedio de sum
				sprintf((char*)msg2, "Promedio: %.2f (usando %lu valores)\r\n", promedio, items);
			}

			HAL_UART_Transmit(&huart3, msg2, strlen((char*)msg2), 1000);
			osMutexRelease(uartMutexHandle);
		}
		osDelay(250);
	}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	osSemaphoreRelease(buttonSemHandle); //Libera el Semaphore! Esto es atendido en la task que estaba esperando al SEMAPHORE
}
