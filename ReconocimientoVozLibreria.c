...
/*A esta funcion van a entrar los tres arreglos calculados con distancia relativa*/
int definirPersona(volatile float *adrian,volatile float *alan,volatile float *diego){
	...
	//Manda señal sobre quiene esta hablando
	/*Adrian=0
	 *Diego=1
	 *Alan=2
	 *Ninguno=3*/
	...
	else if(sumaDiego==min){
		return 1;
	}
	else if(sumaAlan==min){
		return 2;
	}
	else{
		return 3;
...
int procesar(volatile float *array, int length,
    ...
    for (int i = 0; i + FRAME <= length; i += HOP) {
		...
		/* Si energía muy baja -> considerar frame como silencio y saltarlo
		   (evita amplificar ruido). Ajusta umbral según tu micrófono.
		const float SILENCE_THRESH = 50.0f; // ajustar según ADC/ganancia
		if (maxAbs < SILENCE_THRESH) {
			// Opcional: almacenar ceros en buffers o simplemente saltar
			// Guardamos ceros como features
			if (frameCont < MAX_FRAMES) {
				zcrBuff[frameCont] = 0.0f;
				centroidBuff[frameCont] = 0.0f;
				bandwithBuff[frameCont] = 0.0f;
				rolloffBuff[frameCont] = 0.0f;
			}
			frameCont++;
