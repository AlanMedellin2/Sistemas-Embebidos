...
void Red_Task(void *argument){
	uint32_t items = osMessageQueueGetCount(queueSpeedHandler);
	uint32_t itemsCounter = items;
	uint16_t newValue = 0;
	uint32_t sum = 0;
	for(;;){
		sum = 0;
		...
		while (items > 0 ){
			if (osMessageQueueGet(queueSpeedHandler, &newValue, NULL, 6000) == osOK){
				sum += newValue;
			}
			items = osMessageQueueGetCount(queueSpeedHandler);
		}
		...
		if (osMutexAcquire(uartMutexHandle, 500) == osOK){
			if (itemsCounter == 0)
			{
				sprintf((char*)msg, "No hay valores disponibles.\r\n");
			}
			else
			{
				//Sacar promedio de sum
				sprintf((char*)msg, "Promedio: %.2d (usando %d valores)\r\n", sum, itemsCounter);
			}
