...
int main(void)
{
  ...
  while (1)
  {
    uint8_t result = check_fingerprint();
        ...
        if (result == (uint8_t)-1) {
            // No existe
            uint8_t msg[] = "Huella no registrada\r\n";
            HAL_UART_Transmit(&huart3, msg, sizeof(msg) - 1, HAL_MAX_DELAY);
        }
        else if (result == (uint8_t)-2) {
            uint8_t msg[] = "Error en lectura\r\n";
            HAL_UART_Transmit(&huart3, msg, sizeof(msg) - 1, HAL_MAX_DELAY);
        }
        else if (result == (uint8_t)-3) {
            uint8_t msg[] = "No hay dedo o imagen invalida\r\n";
            HAL_UART_Transmit(&huart3, msg, sizeof(msg) - 1, HAL_MAX_DELAY);
        }
        else {
            uint8_t msg[] = "Huella reconocida!\r\n";
            HAL_UART_Transmit(&huart3, msg, sizeof(msg) - 1, HAL_MAX_DELAY);
        }
